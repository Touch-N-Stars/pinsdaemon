...
- As a mixed-package compatibility safeguard, a missing or invalid viability helper cannot reject ordinary router Wi-Fi: profile activation accepts a valid interface-specific default route and the watchdog keeps using its interface-bound gateway ping. Gatewayless networks still fail closed and require the complete verified-PINS-peer path.
